...
const userController = {
    ...
    getAllUser: async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(400).json({
                error: 'Only the admin can see all the users!'
            });
        try {
    ...
    getUserInfoById: async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(400).json({
                error: 'Only the admin can see the user'
            });
    ...
    updateById: async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(400).json({ error: 'Only the admin can update user' });
    ...
    deleteById: async (req, res) => {
        if (req.user.role !== 'admin')
            return res.status(400).json({
                error: 'Only the admin can delete user'
            });
